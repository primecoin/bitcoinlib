...
class PrimecoindClient(BaseClient):
    """
    Class to interact with primecoind, the Primecoin deamon
    """

    @staticmethod
    def from_config(configfile=None, network='primecoin'):
        """
        Read settings from primecoind config file

        :param configfile: Path to config file. Leave empty to look in default places
        :type: str
        :param network: Primecoin mainnet or testnet. Default is primecoin mainnet
        :type: str

        :return PrimecoindClient:
        """
        try:
            config = configparser.ConfigParser(strict=False)
        except TypeError:
            config = configparser.ConfigParser()
        config_fn = 'primecoin.conf'
        if isinstance(network, Network):
            network = network.name
        if network == 'primecoin_testnet':
            config_fn = 'primecoin_testnet.conf'
        if not configfile:
            cfn = os.path.join(os.path.expanduser("~"), '.bitcoinlib/config/%s' % config_fn)
            if not os.path.isfile(cfn):  # Linux
                cfn = os.path.join(os.path.expanduser("~"), '.primecoin/%s' % config_fn)
            if not os.path.isfile(cfn):  # Try Windows path
                cfn = os.path.join(os.path.expanduser("~"), 'Application Data/Primecoin/%s' % config_fn)
            if not os.path.isfile(cfn):  # Try Mac path
                cfn = os.path.join(os.path.expanduser("~"), 'Library/Application Support/Primecoin/%s' % config_fn)
            if not os.path.isfile(cfn):
                raise ConfigError("Please install primecoin client and specify a path to config file if path is not "
                                  "default. Or place a config file in .bitcoinlib/config/primecoin.conf to reference to "
                                  "an external server.")
        else:
        ...
        return PrimecoindClient(network, url)

    def __init__(self, network='primecoin', base_url='', denominator=100000000, *args):
        """
        Open connection to primecoin node

        :param network: Primecoin mainnet or testnet. Default is primecoin mainnet
        :type: str
        :param base_url: Connection URL in format http(s)://user:password@host:port.
        :type: str
        :param denominator: Denominator for this currency. Should be always 100000000 (satoshis) for primecoin
        :type: str
        """
        if isinstance(network, Network):
            network = network.name
        ...
        if len(base_url.split(':')) != 4:
            raise ConfigError("Primecoind connection URL must be of format 'http(s)://user:password@host:port,"
                              "current format is %s. Please set url in providers.json file or check primecoin config "
                              "file" % base_url)
        if 'password' in base_url:
            raise ConfigError("Invalid password in primecoind provider settings. "
                              "Please replace default password and set url in providers.json or primecoin.conf file")
        _logger.info("Connect to primecoind on %s" % base_url)
    ...
    def getutxos(self, address, after_txid='', max_txs=MAX_TRANSACTIONS):
        ...
        if not (res['ismine'] or res['iswatchonly']):
            raise ClientError("Address %s not found in primecoind wallet, use 'importaddress' to add address to "
                              "wallet." % address)
    ...
    def estimatefee(self, blocks):
        ...
        try:
            ...
            _logger.warning("primecoind error: %s, %s" % (e, pres))
...
if __name__ == '__main__':
    ...
    # base_url = 'http://primecoinrpc:passwd@host:9912'
    # bdc = PrimecoindClient(base_url=base_url)

    # 2. Or connect using default settings or settings from config file
    bdc = PrimecoindClient()
    ...
    t = bdc.getrawtransaction('7213e4d0a01d4858cf82be438deb21359d86281d39634b8c5053cdf6d02452ee')
    pprint(t)
